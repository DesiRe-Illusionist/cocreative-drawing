...
from . import model as sketch_rnn_model
from . import utils
import numpy as np
import requests
import six
...
tf.app.flags.DEFINE_boolean(
    "resume_training", True, "Set to true to load previous checkpoint"
)
tf.app.flags.DEFINE_string(
    "hparams",
    "",
...
def download_pretrained_models(
    ...
    if os.path.isfile(zip_path):
        ...
        tf.logging.info(
            "Downloading pretrained models from %s...", pretrained_models_url
        )
...
def train(sess, model, eval_model, train_set, valid_set, test_set):
    ...
    tf.logging.info(f"{hps.save_every}, {hps.num_steps}")
    for _ in range(hps.num_steps):
        ...
        curr_kl_weight = (
            hps.kl_weight
            - (hps.kl_weight - hps.kl_weight_start) * (hps.kl_decay_rate) ** step
        )
        ...
        if step % hps.save_every == 0 and step > 0:
            ...
            output_values = (
                min(best_valid_cost, valid_cost),
                valid_cost,
                valid_r_cost,
                valid_kl_cost,
                time_taken_valid,
...
def trainer(model_params):
    ...
    with tf.gfile.Open(os.path.join(FLAGS.log_root, "model_config.json"), "w") as f:
        # tf.logging.info('!!!!!!!!!!!!!!!!!!!!!! Writing model params')
        # tf.logging.info(model_params.values())
        # tf.logging.info(list(model_params.values()))
        # tf.logging.info('!!!!!!!!!!!!!!!!!!!!!!')
