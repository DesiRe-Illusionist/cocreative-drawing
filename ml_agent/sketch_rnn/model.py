...
class Model(object):
    ...
    def build_model(self, hps):
        ...
        if use_input_dropout:
            tf.logging.info(
                "Dropout to input w/ keep_prob = %4.4f.", self.hps.input_dropout_prob
            )
        ...
        if use_output_dropout:
            tf.logging.info(
                "Dropout to output w/ keep_prob = %4.4f.", self.hps.output_dropout_prob
            )
            cell = tf.nn.rnn_cell.DropoutWrapper(
        ...
        if hps.conditional:  # vae mode:
            ...
            pre_tile_y = tf.reshape(
                self.batch_z, [self.hps.batch_size, 1, self.hps.z_size]
            )
        ...
        def get_lossfunc(
            ...
            result0 = tf_2d_normal(
                x1_data, x2_data, z_mu1, z_mu2, z_sigma1, z_sigma2, z_corr
            )
            epsilon = 1e-6
